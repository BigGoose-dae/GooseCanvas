...
func modelFromRequest(req modelRequest) (domain.ModelDefinition, error) {
	...
	if req.TaskType != "image" && req.TaskType != "video" {
		return domain.ModelDefinition{}, fmt.Errorf("任务类型仅支持 image 或 video")
	}
	if req.Provider == "" {
		req.Provider = "volcengine"
	}
	if req.Protocol == "" {
		req.Protocol = map[string]string{"image": "ark-image-v3", "video": "ark-video-v3"}[req.TaskType]
	}
	expectedProtocol := map[string]string{"image": "ark-image-v3", "video": "ark-video-v3"}[req.TaskType]
