...
Goose Canvas 是一个开源、本地优先的 AI 图片与视频创作画布。你可以在无限画布中组织文本、图片和视频节点，将素材与提示词连接成工作流，并通过火山引擎方舟模型完成内容生成。
...
3. 从素材或文本节点右侧的连接点拖线到生成节点。
4. 选中生成节点，设置模型、提示词、画面比例、清晰度和时长等参数。
5. 点击「开始生成」。任务会进入后端队列，并通过 SSE 实时更新状态。
6. 生成完成后，可以在节点、任务队列或生成历史中预览和下载结果。
...
模型管理页面支持配置模型 Key、任务类型、供应商、调用协议、输入类型、默认参数和可选参数。目前支持 `volcengine` 供应商，以及 `ark-image-v3` 和 `ark-video-v3` 协议。
...
图片 API 返回 Base64 内容时，服务会解码并保存到本地。图片或视频 API 返回临时文件链接时，服务会立即下载并归档到本地目录，因此生成结果不依赖第三方临时链接长期有效。
