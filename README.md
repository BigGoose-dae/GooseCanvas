...
Goose Canvas 是一个开源、本地优先的多媒体创作画布。你可以在无限画布中组织文本、图片、音频和视频节点，将素材与提示词连接成工作流，并通过火山引擎模型生成文本、图片和视频。
...
- 使用 Doubao Seed 2.1 Pro 生成、改写或扩展文本，并支持文生图、图生图、文生视频和图生视频。
- 上传本地图片、音频和视频。上传音频后会创建可播放、复制和下载的音频节点；当前暂不开放音频模型生成。
...
- 测试方舟连接并同步当前凭证与项目对应的 Seedance 可信素材组和素材；可用素材加入画布时会下载到本地用于预览和归档，并保留 `asset://` 引用参与视频生成。
...
1. 在首页创建一个项目。
2. 进入画布，添加文本、图片或视频节点，也可以直接上传图片、音频和视频素材；上传音频会自动创建音频节点。
3. 文本节点正文与生成提示词分别保存：在节点中编辑正文，在右侧参数面板填写提示词；也可以从上游文本节点连线补充上下文。素材或文本节点可以连接到图片和视频节点。
4. 选中可生成节点，设置模型、提示词或文本指令，以及画面比例、清晰度和时长等参数。音频节点仅提供文件播放、复制、历史和下载能力。
5. 点击「开始生成」。任务会进入后端队列，并通过 SSE 实时更新状态。
6. 生成完成后，可以在节点或生成历史中查看文本结果，并预览或下载图片和视频结果；上传的音频可直接在节点中播放或下载。
...
模型管理页面支持配置模型 Key、任务类型、供应商、调用协议、输入类型、默认参数和可选参数。目前支持 `volcengine` 供应商，以及 `ark-chat-v3`、`ark-image-v3` 和 `ark-video-v3` 协议。
...
文本结果会写回节点并保存到生成历史。图片 API 返回 Base64 内容时，服务会解码并保存到本地。图片或视频 API 返回临时文件链接时，服务会立即下载并归档到本地目录，因此生成结果不依赖第三方临时链接长期有效。上传的音频文件同样保存在本地素材目录中。
