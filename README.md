...
Goose Canvas 是一个开源、本地优先的 AI 文本、图片与视频创作画布。你可以在无限画布中组织文本、图片和视频节点，将素材与提示词连接成工作流，并通过火山引擎方舟模型完成内容生成。

## 核心功能

- 创建和管理多个创作项目。
- 在无限画布中添加、拖动、缩放、重命名、复制和删除节点。
- 使用文本、图片和视频节点组织提示词与素材。
- 使用 Doubao Seed 2.1 Pro 生成、改写或扩展文本，并支持文生图、图生图、文生视频和图生视频。
- 上传本地图片和视频，并通过节点连线作为生成输入。
...
3. 文本节点可直接输入写作指令，也可以从上游文本节点连线补充上下文；素材或文本节点也可以连接到图片、视频节点。
4. 选中节点，设置模型、提示词或文本指令，以及画面比例、清晰度和时长等参数。
5. 点击「开始生成」。任务会进入后端队列，并通过 SSE 实时更新状态。
6. 生成完成后，可以在节点或生成历史中查看文本结果，并预览或下载图片、视频结果。
...
文本结果会写回节点并保存到生成历史。图片 API 返回 Base64 内容时，服务会解码并保存到本地。图片或视频 API 返回临时文件链接时，服务会立即下载并归档到本地目录，因此生成结果不依赖第三方临时链接长期有效。
